# app.py v4.2 — Football Predictor Pro
...
.status-run{background:#0c1a3a;border:1px solid #3b82f6;border-radius:10px;padding:.5rem 1rem;font-size:.85rem;color:#93c5fd;}
...
.chip{display:inline-block;background:#1e293b;border:1px solid #334155;border-radius:20px;padding:3px 10px;font-size:.76rem;margin:2px;color:#94a3b8;}
...
def friendly_comp(comp_id):
    ...
    slug  = parts[0]; yr = parts[1] if len(parts)>1 else ''
...
def render_prediction(pred, home, away, hs, as_, compact=False):
    ...
    # Top 5 marcadores
    st.markdown("<div class='section-hdr'>Top 5 marcadores más probables</div>", unsafe_allow_html=True)
    ...
    if pred.get('has_xg') and pred.get('xg_data'):
        ...
        x2.metric(f'Suerte {home[:12]}',  xd.get('home_luck','—'),
                  help='Positivo = sobrerendimiento, posible regresión')
        x3.metric(f'xGF/pj {away[:12]}', xd.get('away_xgF_pg','—'))
        x4.metric(f'Suerte {away[:12]}',  xd.get('away_luck','—'))
        src = orch.state.get('xg_source','—')
        st.caption(f'xG: {src} · Lambda = 50% goles reales + 50% xG esperados')

# ════ SIDEBAR ════
with st.sidebar:
    st.markdown('## ⚽ Football Predictor Pro')
    st.caption(f"v4.2 · {catalog.get('total_comps',0)} ligas")
    if orch.system.trained:
        ...
        st.markdown(
            ...
            f"{cname} {s_lbl}&nbsp;"
            f"<span style='color:#86efac'>{xg_ic} xG:{xg_nm} · CV:{cv_v:.0%} · {played}pj</span>"
            f"</div>", unsafe_allow_html=True)
        ls = orch.state.get('last_sync_time','')
        if ls: st.caption(f'Sync: {ls}')
    else:
    ...
    c1,c2 = st.columns(2)
    with c1: sync_min_btn = st.button('⚡ Sync Mínimo', use_container_width=True, help='Liga activa + xG')
    with c2: sync_deep_bg = st.button('🌍 Sync Deep',   use_container_width=True, help='Todas las ligas en background')
    force_dl = st.checkbox('Forzar re-descarga', key='force_dl')
    if sync_deep_bg:
        if not orch._deep_running:
            orch.sync_deep_background(force=force_dl)
...
with tab_search:
    ...
    if orch._deep_running:
        ...
        st.markdown(
            f"<div class='status-run'>⚡ Sync Deep: {s2.get('deep_message','')} — {prog:.0%}</div>",
        ...
        if is_match_q:
            ...
            if mi:
                ...
                if is_same:
                    if st.button('⚡ Analizar este partido ahora', type='primary', use_container_width=True):
                        ...
                        if analysis:
                            ...
                            st.info('💡 Tab ⚡ Predicción Manual para análisis completo (heatmap, todos los mercados, xG).')
                else:
                ...
                for lbl_,q_ in [('Local',home_q),('Visitante',away_q)]:
                    ...
                    if hits:
                        st.caption(f"Similares {lbl_}: {', '.join(h['team'] for h in hits)}")
        else:
...
with tab_pred:
    if not orch.system.trained: st.info('Carga una liga desde el sidebar.')
        ...
        with c1:
            home = st.selectbox('Local', teams, index=teams.index(def_home) if def_home in teams else 0, key='pm_h')
            ...
            if analysis:
                ...
                with hm:
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-size:1.3rem'>VS</div>{xg_flag}"
                        f"</div>", unsafe_allow_html=True)
                with h2:
                    st.markdown(
...
with tab_table:
    if not orch.system.trained: st.info('Carga una liga primero.')
        ...
        if df_t.empty: st.warning('Sin datos suficientes.')
            ...
            # Resumen estadístico de la liga
            played_r = df_t['PJ'].sum()//2 if 'PJ' in df_t else 0
            if 'GF' in df_t and 'PJ' in df_t:
                total_g  = df_t['GF'].sum()
                avg_g    = round(total_g/played_r,2) if played_r else 0
                home_w   = round(df_t['PG'].sum()/played_r*50,1) if played_r else 0
                s1,s2,s3,s4 = st.columns(4)
                s1.metric('Partidos jugados', played_r)
                s2.metric('Goles totales',    total_g)
                s3.metric('Goles/partido',    avg_g)
                s4.metric('% Victoria local est.', f'{home_w}%')
    ...
    if not orch.system.trained: st.info('Carga una liga primero.')
        ...
        if std_:
            ...
            if std_.get('has_xg'):
                ...
                x5.metric('Overperf Atq',std_.get('xg_overperf_att','—'), help='Goles−xG. Positivo=suerte, posible regresión')
    ...
    if not orch.system.trained: st.info('Carga una liga primero.')
        ...
        if st.button('Ver H2H', use_container_width=True):
            ...
            if df_.empty: st.info('Sin enfrentamientos previos en los datos cargados.')
                ...
                st.dataframe(df_show[['date','home_team','Score','away_team']].rename(columns={'date':'Fecha','home_team':'Local','away_team':'Visitante'}),use_container_width=True,hide_index=True)
    ...
    if not orch.system.trained: st.info('Carga una liga primero.')
        ...
        if st.button('Ejecutar backtest', use_container_width=True):
            ...
            if df_bt.empty: st.warning('Insuficientes datos.')
                ...
                bt2.metric('Precisión Victoria L', f'{prec_h:.1%}')
                bt3.metric('Precisión Empate',     f'{prec_d:.1%}')
                bt4.metric('Precisión Victoria V', f'{prec_a:.1%}')
...
with tab_xg:
    if not orch.system.trained: st.info('Carga una liga primero.')
        ...
        if xg_avail:
            st.markdown(
                f"<div style='background:#1c2940;border:1px solid #3b82f6;border-radius:10px;padding:1rem 1.2rem;'>"
                f"<b>📊 xG disponible via {xg_avail} para esta liga</b><br>"
                f"Activar xG mejora la precisión del modelo un <b>~4–8%</b>.<br>"
                f"Lambda Poisson = <b>50% goles reales + 50% xG esperados</b> de {xg_avail}.<br>"
                f"<small style='color:#64748b'>Los datos xG se descargan automáticamente y se guardan en caché.</small>"
                f"</div>", unsafe_allow_html=True)
            st.markdown('')
            if st.button('⚡ Activar xG ahora (re-entrena)', type='primary', use_container_width=True):
                train_yrs_xg = [season_opts[l] for l in train_labels if l in season_opts]
                pred_yr_xg   = season_opts.get(predict_lbl, 0)
                pb_xg=st.progress(0.0); sm_xg=st.empty()
                def _pf_xg(msg,v): sm_xg.caption(msg); pb_xg.progress(float(v))
                ok_xg,msg_xg = orch.sync_minimum(slug,train_yrs_xg,pred_yr_xg,force=True,progress_fn=_pf_xg)
                pb_xg.empty(); sm_xg.empty()
                if ok_xg and orch.system.has_xg: st.success('✅ xG activado'); st.rerun()
                elif ok_xg: st.warning('Re-entrenado pero xG no disponible para esta liga.')
                else: st.error(f'Error: {msg_xg}')
        else:
            st.warning('Esta liga no tiene cobertura xG (Understat: EPL/Bundesliga/La Liga/Serie A/Ligue 1 · FBref: +21 ligas).')
    else:
        ...
        if df_xg_t.empty: st.info('Sin datos xG suficientes.')
            ...
            fig_xg = go.Figure()
            fig_xg.add_trace(go.Bar(name='xG Diff',x=df_xg_t['Equipo'],y=df_xg_t['xG_Diff'],marker_color='#3b82f6'))
            fig_xg.add_trace(go.Bar(name='GD Real',x=df_xg_t['Equipo'],y=df_xg_t['GF']-df_xg_t['GA'],marker_color='#94a3b8',opacity=0.6))
            fig_xg.update_layout(barmode='group',height=300,plot_bgcolor='#0f172a',paper_bgcolor='#0f172a',font_color='white',margin=dict(t=20,b=10))
            st.plotly_chart(fig_xg, use_container_width=True)
            st.markdown('#### Luck Index')
            st.caption('Sup-derecha = suerte positiva → posible caída. Inf-izquierda = merece más.')
            ...
            fig_luck.update_layout(height=480,plot_bgcolor='#0f172a',paper_bgcolor='#0f172a',font_color='white',
                xaxis_title='Overperf Ataque (Goles−xG)',yaxis_title='Overperf Defensa (xGC−GC)',margin=dict(t=20))
        ...
        if xg_s:
            x1,x2,x3,x4,x5,x6 = st.columns(6)
            x1.metric('xGF',xg_s.get('xGF','—')); x2.metric('xGA',xg_s.get('xGA','—'))
            x3.metric('xGF/pj',xg_s.get('xGF_per_game','—')); x4.metric('xGA/pj',xg_s.get('xGA_per_game','—'))
            x5.metric('Overperf Atq',xg_s.get('overperf_attack','—'),help='Goles−xG. Positivo = suerte positiva.')
            x6.metric('Luck Index',xg_s.get('luck_index','—'))
            # Trend xG (mejora extra)
            df_xg_trend = orch.system.xg_team_trend(team_xg) if hasattr(orch.system,'xg_team_trend') else None
            if df_xg_trend is not None and not df_xg_trend.empty:
                st.markdown('**Tendencia xG — últimos 10 partidos**')
                fig_tr = go.Figure()
                fig_tr.add_trace(go.Scatter(x=df_xg_trend['date'],y=df_xg_trend['xGF'],name='xGF',line=dict(color='#4ade80')))
                fig_tr.add_trace(go.Scatter(x=df_xg_trend['date'],y=df_xg_trend['xGA'],name='xGA',line=dict(color='#f87171')))
                fig_tr.add_trace(go.Scatter(x=df_xg_trend['date'],y=df_xg_trend['gf'], name='Goles F',line=dict(color='#4ade80',dash='dot')))
                fig_tr.update_layout(height=260,plot_bgcolor='#0f172a',paper_bgcolor='#0f172a',font_color='white',margin=dict(t=20,b=10))
                st.plotly_chart(fig_tr, use_container_width=True)
        else: st.info('Sin datos xG para este equipo.')
